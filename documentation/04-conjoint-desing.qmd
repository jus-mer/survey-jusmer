...
library(here)
here::i_am("documentation/04-conjoint-desing.qmd")
...
# Statistical Design: Distributive Conjoint Pre-Plan

## Introduction

Entitlement to welfare provision is rarely unconditional. Even in solidaristic systems, access to social goods rests on normative judgments about who deserves collective support and on what grounds, what welfare state research terms conditional solidarity and, at the individual level, deservingness [@oorschot_who_2000; @meuleman_welfare_2020]. A growing experimental literature has established how citizens weigh criteria such as need, control, reciprocity, effort, attitude and identity, overwhelmingly in the domain of unemployment benefits and social assistance: respondents rate how deserving a fictional claimant is, or state what share of a benefit that claimant should receive, one claimant at a time [@knotz_recast_2022; @meuleman_welfare_2020], or decide which of two claimants to prioritize for a service [@dietrich_prioritizing_2026]. Both formats evaluate claimants against a standard rather than against each other, which allows respondents to endorse each criterion independently without ever resolving the conflicts between them. Yet justice principles routinely point in opposite directions—a claimant may be needy but exert little effort, another may have contributed much but need little—so that honoring need and merit at once is a dilemma rather than a sum [@gilgen_disentangling_2022]. Only when a finite resource must be divided is a respondent forced to adopt a weighting among the competing principles, and it is that weighting, rather than the separate endorsement of each criterion, that constitutes a distributive preference. A division task also makes observable a principle the standard formats cannot register at all: equality, since an equal split is expressible only where respondents control how much each claimant receives.

We therefore administer a pre-registered paired-profile distributive conjoint experiment to a Chilean sample, adapting Gilgen's [-@gilgen_disentangling_2022] Distributional Survey Experiment to the tabular profile presentation and the complete, independent randomization of a conjoint design. Respondents act as members of a university committee dividing a fixed first-year scholarship fund between two applicants via a continuous slider, so that giving more to one means giving less to the other. Applicant profiles are built on the recast NICER scheme [@knotz_recast_2022], which integrates and refines van Oorschot's CARIN criteria, and vary need, control, effort, reciprocity, attitude, identity, and sex. The setting further moves deservingness research into a domain where it has rarely been tested experimentally: access to higher education rather than unemployment or social assistance. Chile is a demanding case. Its higher education system combines some of the highest shares of private household expenditure in the OECD with acute stratification by socioeconomic origin [@valenzuela_socioeconomic_2013], and its population holds unusually strong meritocratic beliefs relative to its level of inequality [@castillo_meritocracia_2019]. If need dominates allocations even here, its primacy as a distributive principle appears robust; if effort and control prevail, this reveals how commodification and meritocratic ideology jointly reshape distributive judgments.
...
As a robustness check for the fixed-sum structure, we estimate a within-task difference model, regressing the difference in shares between the two profiles in a task on the differences in their attributes:

$$\text{share}_{it}^{A} - \text{share}_{it}^{B} = \sum_l \beta_l\left(T^{(l),A}_{it} - T^{(l),B}_{it}\right) + \varepsilon_{it},$$

with standard errors clustered by respondent. This collapses each task to a single observation, resolving by construction the exact dependence between the two shares. Because a one-level change raises one applicant's share and lowers the other's, coefficients in this specification are approximately twice those of the profile-level model; the two are equivalent up to that scaling. The same specification, with the attribute differences interacted with the standardized moderator, serves as the robustness check for the conditional effects.

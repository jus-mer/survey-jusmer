...
A different approach is to use a D-efficient design, which is a type of experimental design that maximizes the amount of information that can be obtained from a given number of respondents. D-efficient designs are often used in conjoint experiments to reduce the number of profiles that need to be presented to respondents while still allowing for the estimation of the main effects and interactions of interest. Using this approach, we can reduce the number of respondents needed to achieve the same level of statistical power. [acá sigue @tomasurzuam]
